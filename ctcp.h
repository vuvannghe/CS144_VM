 ...
 */
#define MAX_SEG_DATA_SIZE 1440 // Kích thước tối đa của 1 TCP segment chưa tính cả header là giá trị của cả receive window size và send window size trong lab 1
...
typedef struct {
  ...
  int rt_timeout;          /* Retransmission timeout, in ms */ // Thời gian đợi ACK segment để truyền lại
} ctcp_config_t;
